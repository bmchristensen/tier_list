...
RSpec.describe Champion, type: :model do
  ...
  it "can calculate champion score based on all tiers C, B, A, and S (scores 1-4 respectively)" do
    ...
    expect(champion1).to have_score(1)
    expect(champion2).to have_score(2)
    expect(champion3).to have_score(3)
    expect(champion4).to have_score(4)
  end
end
